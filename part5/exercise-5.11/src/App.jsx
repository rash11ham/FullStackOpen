...
const App = () => {
  ...
  const deleteBlog = (id) => {
    ...
    // const confirmDelete = window.confirm(`Are you sure you want to delete the Blog by ${blog.author}`)
    // if(!confirmDelete) return
    blogService
      .remove(id)
      .then(() => {
        setBlogs(blogs.filter(b => b.id !== id))
        setMessage(`blog by ${blog.author} has been removed!`)
        setTimeout(() => {
          setMessage('')
        }, 5000)
      }).catch(error => {
  ...
  const toggleLike = (id) => {
    ...
    blogService
      ...
      .then(updatedBlogLikes => {
        setBlogs(blogs.map(b => b.id !== id ? b : updatedBlogLikes))
        setMessage('thank you for liking a blog')
        setTimeout(() => {
          setMessage('')
        }, 5000)
      }).catch(error => {
        setMessage('Something went worng'+error)
        setTimeout(() => {
          setMessage('')
        }, 5000)
      })
